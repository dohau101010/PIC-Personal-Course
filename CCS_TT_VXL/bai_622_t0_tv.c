...
void glcd_hthi_t0(usi8 addr)
{
   ...
   tinh_t0_ud();
   chuc=t0ud/10%10; donvi=t0ud%10;
   glcd_command(addr); //thiet lap dia chi can hien thi gia tri t0 tren glcd
   if(chuc==0) glcd_data(' ');
...
void dem_xung_ngoai_t0 ()
{
   ...
   if (t0_tam!=t0)
   {
      ...
      hienthi_4led7doan ();
      lcd_hienthi_t0_soto() ;
      glcd_hthi_t0(glcd_addr_line2+7); //hien thi tai hang 2 vi tri thu 7
   }
}
